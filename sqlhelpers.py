...
def send_money(sender, recipient, amount):
    ...
    mine.mine_for_block(data) 
    #--------------------------------------------------------------------------->ntegration
   # os.system('cmd /k "python node.py"')
    sync.sync(save=True)
...
def get_blockchain():
    ...
    blockchain_sql = Table("blockchain", "number", "hash","nonce" , "data","previous")
...
def sync_blockchain(blockchain):
    ...
    for block in blockchain.chain:
        blockchain_sql.insert(str(block.number), block.hash(), block.nonce, block.data,block.previous_hash )
